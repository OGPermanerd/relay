...
2. `generateUniqueSlug(name: string, db: typeof import("@everyskill/db").db): Promise<string>`
...
- Import eq, like from drizzle-orm
  ...
  <verify>
TypeScript compilation passes. Run: `pnpm --filter @everyskill/web exec tsc --noEmit`
  </verify>
  <done>
...
Also update the existing "Browse Skills" card to remain disabled for now (Phase 6).
  ...
  <verify>
1. TypeScript passes: `pnpm --filter @everyskill/web exec tsc --noEmit`
2. Build succeeds: `pnpm --filter @everyskill/web build`
...
<verification>
1. TypeScript compilation: `pnpm --filter @everyskill/web exec tsc --noEmit` passes
2. Build: `pnpm --filter @everyskill/web build` succeeds
