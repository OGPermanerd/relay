...
2. Run pnpm --filter @everyskill/db db:push (tables created)
3. Run pnpm --filter @everyskill/db db:seed (3 skills inserted)
...
3. Human confirms: `docker exec everyskill-postgres psql -U postgres -d relay -c "SELECT count(*) FROM skills;"` returns 3
</verification>
