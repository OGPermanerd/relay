...
Run `pnpm install` from project root.
  ...
  <verify>
    `pnpm --filter @everyskill/mcp vitest --version` shows vitest installed
  </verify>
  <done>
...
**apps/mcp/test/mocks.ts:**
```typescript
import type { Skill } from "@everyskill/db/schema";
...
Note: These tests verify the database query patterns work correctly. The actual MCP protocol handling is tested by the SDK.
  ...
  <verify>
    `pnpm --filter @everyskill/mcp test` passes
  </verify>
  <done>
...
<verification>
1. `pnpm --filter @everyskill/mcp test` - All tests pass
2. `pnpm --filter @everyskill/mcp typecheck` - Test files compile
