...
**package.json:**
- name: "@everyskill/mcp"
- bin: { "everyskill-mcp": "./dist/index.js" }
- scripts: { "dev": "tsx src/index.ts", "build": "tsup src/index.ts --format cjs --dts", "typecheck": "tsc --noEmit" }
- dependencies: @modelcontextprotocol/sdk ^1.25.0, zod ^3.25.0, @everyskill/db workspace:*
...
**CRITICAL:** Never use console.log in stdio servers - it corrupts JSON-RPC protocol.
  ...
  <verify>
    `pnpm --filter @everyskill/mcp dev` starts without error (Ctrl+C to stop)
...
<task type="auto">
  <name>Task 3: Add usageEvents schema to @everyskill/db</name>
  <files>
    packages/db/src/schema/usage-events.ts
    packages/db/src/schema/index.ts
  </files>
...
Run `pnpm --filter @everyskill/db db:push` to sync schema to database.
...
Note: skillId is text (not uuid) to match the skills table that will be created in Plan 02. The reference constraint will be added when skills table exists.
  ...
  <verify>
    ...
    `pnpm --filter @everyskill/db db:push` shows usage_events table created/updated
  </verify>
  <done>
    usageEvents table exists in database schema, types exported from @everyskill/db
  </done>
</task>
...
<verification>
1. `pnpm --filter @everyskill/mcp typecheck` - MCP package compiles
2. `pnpm --filter @everyskill/mcp dev` starts server (use Ctrl+C to exit after ~2 seconds)
3. `pnpm --filter @everyskill/db typecheck` - DB package compiles with new schema
...
- usageEvents table schema exists in @everyskill/db
