...
Run `pnpm --filter @relay/db db:push` to create skills table.
...
```
  ...
  <verify>
    ...
    `pnpm --filter @relay/db db:push` shows skills table created
  </verify>
  <done>
...
import { usageEvents, type NewUsageEvent } from "@relay/db/schema";
...
This provides a simple `trackUsage()` function that all tools will call. Tracking failures are logged but don't break tool execution.
  ...
  <verify>
    ...
    Import resolves: @relay/db and @relay/db/schema
  </verify>
  <done>
...
```
  ...
  <verify>
    `pnpm --filter @relay/mcp typecheck` passes
    `pnpm --filter @relay/mcp dev` starts without errors
  </verify>
  <done>
...
<verification>
1. `pnpm --filter @relay/db typecheck` - DB compiles with skills schema
2. `pnpm --filter @relay/mcp typecheck` - MCP compiles with tools
3. `pnpm --filter @relay/mcp dev` starts server (tools registered)
