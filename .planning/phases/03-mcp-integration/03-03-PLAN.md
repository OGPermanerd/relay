...
After adding, verify all three tools are properly registered by starting the server.
  ...
  <verify>
    `pnpm --filter @relay/mcp typecheck` passes
    `pnpm --filter @relay/mcp dev` starts without errors (Ctrl+C to exit)
...
<verification>
1. `pnpm --filter @relay/mcp typecheck` - MCP compiles with deploy tool
2. `pnpm --filter @relay/mcp dev` - Server starts with all tools
