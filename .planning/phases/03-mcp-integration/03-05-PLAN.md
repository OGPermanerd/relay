...
must_haves:
  truths:
    - "README documents how to register relay-mcp with Claude Desktop"
...
<objective>
Create documentation and configuration templates for integrating relay-mcp with Claude Desktop.
...
2. Register "relay-mcp" as the server name
...
Note: User must replace "/path/to/relay" with their actual repository path.
  ...
  <done>Example configuration file contains mcpServers.relay-mcp with command and args</done>
</task>
...
In Claude Desktop, you should now see the relay-mcp tools available. Try:
...
3. Run `pnpm --filter @relay/db db:push` to create tables
  </action>
...
<verification>
1. `cat apps/mcp/claude_desktop_config.example.json | jq .` - Valid JSON with mcpServers.relay-mcp
