...
2. Import: auth from @/auth, db and ratings from @everyskill/db, updateSkillRating from @everyskill/db, revalidatePath from next/cache, z from zod, eq and and from drizzle-orm
3. Create ratingSchema with Zod:
   ...
   - hoursSavedEstimate: z.coerce.number().int().min(0).max(1000).optional().transform(val => val || null)
...
13. Hours saved estimate input:
    ...
    - input type="number" name="hoursSavedEstimate", min=0, max=1000, step=1
