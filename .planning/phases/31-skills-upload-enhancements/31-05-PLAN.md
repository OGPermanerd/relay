...
must_haves:
  truths:
    ...
    - "AI review summary data enriches similarity embeddings for more accurate future matching"
...
<task type="auto">
  ...
  <action>
    ...
    2. In `apps/web/components/skill-upload-form.tsx`:
       ...
       - Add an `onMessageAuthor` callback prop placeholder that stores the selected skill for messaging (will be wired in Plan 06)
    ...
    IMPORTANT TIMING NOTE (SKILL-07): The enriched embedding regeneration happens AFTER the skill is created and the AI review completes (fire-and-forget). This means the CURRENT upload's similarity check will NOT benefit from the enriched embedding -- only FUTURE uploads checking similarity against this skill will see improved matching. This is acceptable and avoids a complex synchronous embedding flow.

    3. In `apps/web/app/actions/skills.ts` (the `autoGenerateReview` function added in Plan 03):
       - After the review is successfully generated and upserted, regenerate the embedding with enriched text:
         ...
         // NOTE: This benefits FUTURE similarity checks, not the current upload.
         // The current upload already completed its similarity check before AI review ran.
         generateSkillEmbedding(
           skillId, name,
           `${description} ${reviewOutput.summary}`
         ).catch(() => {});
         ```
       - Add a code comment explaining the timing: "// SKILL-07: Enriched embedding improves future similarity detection. Current upload's check already completed."
  ...
  <verify>
    ...
    - Run Playwright tests with specific criteria:
      `cd apps/web && node_modules/.bin/playwright test`
      Verify:
      1. Upload page (/skills/new) loads without errors or hydration warnings
      2. If similar skills exist for a test upload, the similarity pane renders showing skill cards (check for "Similar Skills Found" heading)
      3. No "Semantic match" or "Name match" text appears in the rendered HTML
      4. The upload form remains visible alongside the similarity pane (form fields are not hidden)
  </verify>
  <done>
    ...
    - AI review summary enriches embeddings for better future similarity matching (not current upload)
    - Form is visible alongside pane (not hidden)
    - Build passes, upload page loads, no matchType labels in HTML
  </done>
</task>
...
4. Playwright tests pass: upload page loads, similarity pane renders when similar skills exist, form visible alongside pane, no matchType labels
</verification>

<success_criteria>
- Rich similarity pane shows on upload when similar skills found
- No matchType labels visible to users
- AI review data enriches embeddings after review completion (benefits future uploads, not current)
