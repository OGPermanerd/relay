...
must_haves:
  truths:
    - "Each user has an admin or member role visible in the database"
    - "The first user per tenant is automatically assigned the admin role after migration"
    - "Application code can query any user's role and check if a tenant has existing users"
  artifacts:
