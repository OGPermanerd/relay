...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/web/app/(protected)/skills/new/page.tsx"
      provides: "Server component passing isAdmin prop to SkillUploadForm"
      contains: "isAdmin"
  ...
  key_links:
    ...
    - from: "apps/web/app/(protected)/skills/new/page.tsx"
      to: "apps/web/lib/admin.ts"
      via: "isAdmin(session) called in server component, result passed as prop"
      pattern: "isAdmin\\(session\\)"
    - from: "apps/web/app/(protected)/skills/new/page.tsx"
      to: "apps/web/components/skill-upload-form.tsx"
      via: "isAdmin boolean prop passed to SkillUploadForm"
...
<task type="auto">
  ...
  <action>
**Step 1: Modify the parent server component** (`apps/web/app/(protected)/skills/new/page.tsx`).

This file currently renders `<SkillUploadForm />` with no props. It needs to resolve the admin status server-side and pass it down as a prop.
...
2. Make the default export an `async` function (it's a server component, so this is fine):
```typescript
...
3. Pass the prop to the form component:
```tsx
...
**Step 2: Update SkillUploadForm props** (`apps/web/components/skill-upload-form.tsx`).

Add `isAdmin?: boolean` to the component's props interface/type. Default to `false` if not provided:
```typescript
// In the props type:
isAdmin?: boolean;
...
**Step 3: Add Private radio button** after the Personal radio (around line 225):
```tsx
...
**Step 4: Add Global radio button** (conditionally shown for admins):
```tsx
{isAdmin && (
  <label className="flex items-center gap-2 cursor-pointer">
    <input
...
**Step 5: Update label descriptions** for existing options to be clearer:
- Team (tenant): "Visible to everyone in your organization"
- Personal: "Only you can see it, appears in your portfolio"

**Step 6: Update the `fields` default** -- keep `visibility: "tenant"` as default (unchanged, this is correct).
  </action>
  <verify>
...
Run the relevant Playwright E2E test for the upload page: `cd /home/dev/projects/relay/apps/web && npx playwright test tests/e2e/upload.spec.ts` (if it exists). If no specific test exists, run `npx playwright test` to verify no regressions.
  ...
  <done>
Parent server component resolves admin status via auth() + isAdmin() and passes it as prop. Upload form shows Team, Personal, Private options for all users plus Global for admins. Default is Team. Form submits correct visibility value.
  </done>
</task>
...
<success_criteria>
- Parent server component (page.tsx) calls auth() + isAdmin() and passes boolean prop
