...
<task type="auto">
  ...
  <action>
Add zod to @everyskill/db dependencies:
```bash
pnpm --filter @everyskill/db add zod
```
...
This allows importing validation schemas from @everyskill/db:
```typescript
import { validateSkillMetadata, SkillMetadata } from "@everyskill/db";
```
  </action>
  <verify>
...
<success_criteria>
- [ ] zod added to @everyskill/db dependencies
