...
user_setup:
  - service: cloudflare-r2
    ...
    env_vars:
      ...
      - name: R2_BUCKET_NAME
        source: "Cloudflare Dashboard -> R2 -> Create bucket (e.g., 'relay-skills')"
    dashboard_config:
...
Output: @relay/storage package with presigned URL generation for uploads and downloads
</objective>
...
- main: "./src/index.ts"
- types: "./src/index.ts"
- exports pattern matching @relay/db
- dependencies:
...
Create tsconfig.json extending ../../tsconfig.json (copy pattern from @relay/db).
...
Run pnpm install to link the new package.
  ...
  <verify>
    pnpm install && pnpm --filter @relay/storage typecheck
  </verify>
  <done>
...
Follow graceful null-handling pattern from @relay/db (returns null when not configured).
  </action>
  <verify>
    pnpm --filter @relay/storage typecheck && pnpm --filter @relay/storage lint
  </verify>
  <done>
...
<verification>
1. Run `pnpm --filter @relay/storage typecheck` - should pass
2. Run `pnpm --filter @relay/storage lint` - should pass
