...
must_haves:
  ...
  artifacts:
    - path: "apps/web/lib/analytics-queries.ts"
      ...
      exports: ["getOverviewStats", "getUsageTrend", "getEmployeeUsage", "getSkillUsage", "getExportData"]
  key_links:
...
Purpose: Provide the data layer for all analytics views (overview stats, usage trends, employee table, skills leaderboard, and CSV export).
...
**Pattern to follow:** See my-leverage.ts for the exact SQL template syntax and result parsing.
  ...
  <done>
    - analytics-queries.ts exists with 5 query functions
...
- 5 main query functions + 2 helper functions exported
