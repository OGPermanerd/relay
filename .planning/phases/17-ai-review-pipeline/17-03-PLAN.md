...
<task type="auto">
  <name>Task 1: Create AI review display and tab components</name>
  <files>
    apps/web/components/ai-review-display.tsx
    apps/web/components/ai-review-tab.tsx
  </files>
  <action>
    **1. Create `apps/web/components/ai-review-display.tsx`:**
    ...
    - Import `ReviewCategories` type from `@relay/db/schema`
    ...
    **2. Create `apps/web/components/ai-review-tab.tsx`:**

    A "use client" component that manages the review trigger, loading state, and visibility toggle.

    - **"use client"** directive
    - Import `useActionState` from "react", `requestAiReview` and `toggleReviewVisibility` from `@/app/actions/ai-review`
    ...
    - **State management:**
      ...
      - `useActionState(toggleReviewVisibility, { })` for the visibility toggle form
    ...
    - **Layout:**
      ...
      - **Error state:** If state.error, show error message in a subtle alert (text-red-600 bg-red-50 rounded p-3)
  ...
  <verify>
    Run `pnpm build --filter web` to verify both components compile. Check that all imports resolve.
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: Integrate AI review tab into skill detail page</name>
  ...
  <action>
    ...
    1. **Add data fetching** (in the existing parallel Promise.all or separately):
       - Import `getSkillReview` from `@relay/db/services`
