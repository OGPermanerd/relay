...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/web/app/actions/ai-review.ts"
      ...
      exports: ["requestAiReview", "AiReviewState"]
  key_links:
...
<task type="auto">
  ...
  <action>
    ...
    4. **toggleReviewVisibility server action:**
       - Signature: `async function toggleReviewVisibility(prevState: AiReviewState, formData: FormData): Promise<AiReviewState>`
       - Extract skillId and isVisible from formData
       - Auth check (must be skill author)
       - Call `toggleReviewVisibility` service function
       - Revalidate path
       - Return success
  ...
  <done>
    ...
    - toggleReviewVisibility server action exists for hiding/showing reviews
  </done>
</task>
...
- toggleReviewVisibility allows author to hide/show their review
