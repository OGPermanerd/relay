...
3. Update `judgeQuality` function signature to accept `skillContent: string` as the first parameter after `client`. Note: `runBenchmark` already receives `params.skillContent` -- the only change is adding `skillContent` as a parameter to `judgeQuality` and passing it from the existing call site. No "threading" through intermediate functions is needed.
...
6. Update the `judgeQuality` call site in `runBenchmark` to pass `params.skillContent` (which already exists on the params object -- no new data fetching needed):
   ```typescript
...
IMPORTANT: Do NOT modify `qualityScore` calculation or the summary stats computation (bestModel/cheapestModel). Those remain exactly as-is per research anti-pattern guidance.
  ...
  <verify>
    ...
    - Manually verify: `judgeQuality` call site passes `params.skillContent` as the second argument (not a new variable -- `params.skillContent` is already available in `runBenchmark`'s scope)
  </verify>
