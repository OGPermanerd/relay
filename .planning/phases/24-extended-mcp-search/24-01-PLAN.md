...
Purpose: Centralize search logic so both MCP stdio and web remote MCP can call a single service instead of duplicating in-memory filtering. Web search (`apps/web/lib/search-skills.ts`) already matches author/tags via ILIKE — this brings MCP to parity. Embedding fallback (CONTEXT.md hybrid search) is deferred to Claude's discretion within the service; MCP stdio lacks VOYAGE_API_KEY so ILIKE-only is the practical default.
...
1. **escapeLike helper** — escapes `%`, `_`, and `\` characters in user input to prevent ILIKE injection:
```typescript
