...
Purpose: This is the core refactoring -- 19 files that currently hardcode or fallback to DEFAULT_TENANT_ID will instead derive tenant from the authenticated session. This eliminates the most common anti-pattern across the codebase.
...
<task type="auto">
  <name>Task 1: Replace DEFAULT_TENANT_ID in all server action files and homepage</name>
  <files>apps/web/app/actions/skills.ts, apps/web/app/actions/ratings.ts, apps/web/app/actions/fork-skill.ts, apps/web/app/actions/api-keys.ts, apps/web/app/actions/user-preferences.ts, apps/web/app/actions/notification-preferences.ts, apps/web/app/actions/export-claude-md.ts, apps/web/app/actions/skill-messages.ts, apps/web/app/actions/discover.ts, apps/web/app/actions/get-skill-content.ts, apps/web/app/actions/ai-review.ts, apps/web/app/actions/submit-for-review.ts, apps/web/app/(protected)/page.tsx</files>
  <action>
For EACH of the 10 server action files that use DEFAULT_TENANT_ID (skills.ts, ratings.ts, fork-skill.ts, api-keys.ts, user-preferences.ts, notification-preferences.ts, export-claude-md.ts, skill-messages.ts, discover.ts, get-skill-content.ts), apply this pattern:
...
- **skills.ts** (6 uses): Has 3 `generateSkillEmbedding` calls that Plan 01 already updated to pass `tenantId`. Remove the transitional `?? DEFAULT_TENANT_ID` fallback from the tenantId variable if Plan 01 added one. The `tenantId` variable from step 3 above should be used everywhere. Also has 1 `upsertSkillReview` call (line 133) -- ensure it passes `tenantId` in the params object.
...
**Also update 2 server action files that call upsertSkillReview but don't currently use DEFAULT_TENANT_ID:**

- **ai-review.ts**: The `requestAiReview` function calls `upsertSkillReview` at line 115 but does NOT currently pass `tenantId`. Add `const tenantId = session.user.tenantId; if (!tenantId) return { error: "Tenant not resolved" };` after the auth check, then add `tenantId` to the `upsertSkillReview({...})` call.
- **submit-for-review.ts**: The `submitForReview` function calls `upsertSkillReview` at line 74 but does NOT currently pass `tenantId`. Add `const tenantId = session.user.tenantId; if (!tenantId) return { error: "Tenant not resolved" };` after the auth check, then add `tenantId` to the `upsertSkillReview({...})` call.
...
**Also update page.tsx (homepage)**: `apps/web/app/(protected)/page.tsx` -- Plan 01 added a transitional `?? DEFAULT_TENANT_ID` fallback for the `getGreeting` call. Remove the fallback and make it strict: `const tenantId = session.user.tenantId; if (!tenantId) redirect("/");` -- then pass `tenantId` to `getGreeting`.
  ...
  <done>All 12 server action files + homepage derive tenantId strictly from session.user.tenantId. No DEFAULT_TENANT_ID references remain in any action file. ai-review.ts and submit-for-review.ts pass tenantId to upsertSkillReview. Missing tenantId returns an error response.</done>
</task>
...
3. All callers of `upsertSkillReview` have been updated in Task 1: `skills.ts` (line 133), `ai-review.ts` (line 115), and `submit-for-review.ts` (line 74) now all pass `tenantId` from session. No other callers exist in the web app.
  </action>
  ...
  <done>All 5 admin/protected server components derive tenantId from session with redirect on failure. skill-reviews.ts requires tenantId (no optional, no inline fallback). All 3 callers (skills.ts, ai-review.ts, submit-for-review.ts) pass tenantId from session. TypeScript compiles cleanly.</done>
</task>
...
4. All 3 upsertSkillReview callers (skills.ts, ai-review.ts, submit-for-review.ts) pass tenantId
5. All files compile without TypeScript errors
6. Every authenticated code path has a tenantId guard that returns error/redirects on missing tenant
</success_criteria>
