...
must_haves:
  truths:
    ...
    - "Internal packages (@relay/core, @relay/db, @relay/ui) resolve without IDE errors"
  artifacts:
    ...
    - path: "packages/core/package.json"
      provides: "@relay/core package definition"
      contains: "@relay/core"
    - path: "packages/db/package.json"
      provides: "@relay/db package definition"
      contains: "@relay/db"
    - path: "packages/ui/package.json"
      provides: "@relay/ui package definition"
      contains: "@relay/ui"
  key_links:
...
<objective>
Scaffold the Relay monorepo with Turborepo, pnpm workspaces, Next.js 15+, and shared packages (@relay/core, @relay/db, @relay/ui). Establish linting, formatting, and pre-commit hooks.
...
<task type="auto">
  ...
  <action>
    ...
    - name: "web"
    - dependencies: next@latest, react@latest, react-dom@latest, @relay/core, @relay/db, @relay/ui (workspace:*)
    - devDependencies: typescript, @types/node, @tailwindcss/postcss
    - scripts: dev, build, start, lint, typecheck

    Create apps/web/next.config.ts with transpilePackages: ["@relay/ui", "@relay/core", "@relay/db"]

    Create apps/web/tsconfig.json extending from base, with paths for @relay/* packages

    Create apps/web/postcss.config.mjs with @tailwindcss/postcss plugin (Tailwind v4 pattern)

    Create apps/web/app/globals.css importing tailwindcss and @relay/ui/styles/globals.css
  ...
  <verify>
    ...
    grep -q "@relay/core" apps/web/package.json
  </verify>
  <done>Next.js app exists with workspace dependencies configured</done>
</task>

<task type="auto">
  <name>Task 3: Create shared packages (@relay/core, @relay/db, @relay/ui)</name>
  <files>
    packages/core/package.json
    packages/core/tsconfig.json
    packages/core/src/index.ts
  ...
  <action>
    ...
    @relay/core:
    - package.json: name "@relay/core", main "./src/index.ts", types "./src/index.ts"
    - tsconfig.json with strict mode
    - src/index.ts exporting placeholder constant (e.g., export const RELAY_VERSION = "0.0.1")

    @relay/db:
    - package.json: name "@relay/db", depends on @relay/core (workspace:*), drizzle-orm, postgres
    ...
    @relay/ui:
    - package.json: name "@relay/ui", depends on @relay/core (workspace:*), react, react-dom
  ...
  <verify>
    grep -q "@relay/core" packages/core/package.json
    grep -q "@relay/db" packages/db/package.json
    grep -q "@relay/ui" packages/ui/package.json
