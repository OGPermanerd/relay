...
must_haves:
  truths:
    ...
    - "Database connection succeeds from @everyskill/db client"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    - Container name: everyskill-postgres
  ...
  <verify>
    docker ps | grep everyskill-postgres
    docker exec everyskill-postgres pg_isready -U postgres
  </verify>
...
<task type="auto">
  ...
  <action>
    ...
    - pnpm --filter @everyskill/db add drizzle-orm postgres
    - pnpm --filter @everyskill/db add -D drizzle-kit
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    ...
    - docker exec -it everyskill-postgres psql -U postgres -d relay -c "\dt"
    - Should show users table
  </action>
  <verify>
    docker exec everyskill-postgres psql -U postgres -d relay -c "\dt" | grep users
  </verify>
...
2. `pnpm --filter @everyskill/db db:push` applies schema without errors
3. `pnpm --filter @everyskill/db db:studio` opens Drizzle Studio
