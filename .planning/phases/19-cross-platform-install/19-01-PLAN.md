...
must_haves:
  truths:
    - "Platform selection modal shows 4 platform cards in a grid (Claude Desktop, Claude Code, Other IDE, Other Systems)"
...
- Add type `Platform = "claude-desktop" | "claude-code" | "other-ide" | "other-systems"`
...
- Platform configs (all use `JSON.stringify(obj, null, 2)`):
  - **claude-desktop** (treated as Claude Desktop per INST-02): stdio config with `npx -y @everyskill/mcp` command. This is the Claude Desktop app that uses `claude_desktop_config.json`.
...
- All configs use the same structure: `{ mcpServers: { "everyskill-skills": { command: "npx", args: ["-y", "@everyskill/mcp"] } } }`
...
- Add function `getConfigInstructions(platform: Platform): string` returning brief setup instruction text per platform:
  ...
  - claude-code: "Add to ~/.claude.json or run: claude mcp add everyskill-skills -- npx -y @everyskill/mcp"
...
- Export function `generateInstallScript(os: DetectedOS): { content: string; filename: string }`
  - For macos/linux: Generate a bash script (`install-everyskill-mcp.sh`) that:
    - Sets `set -e`
    ...
    - Uses `node -e` to JSON-merge `everyskill-skills` into `mcpServers` (preserves existing servers)
    - Prints success message with config path and "Restart Claude Desktop to activate"
  - For windows: Generate a PowerShell script (`install-everyskill-mcp.ps1`) that:
...
- Export function `getRunInstructions(os: DetectedOS): string`
  - macos/linux: `chmod +x install-everyskill-mcp.sh && ./install-everyskill-mcp.sh`
  - windows: `powershell -ExecutionPolicy Bypass -File install-everyskill-mcp.ps1`
  </action>
  <verify>
...
2. **Platform cards grid**: 4 cards in a `grid grid-cols-2 gap-3` layout. Each card is a button with:
   - Icon/emoji (use simple unicode or SVG): Claude Desktop = desktop monitor icon, Claude Code = terminal icon, Other IDE = code brackets icon, Other Systems = server/gear icon
   ...
   - Auto-select platform on mount: after useEffect runs detectOS(), pre-select "claude-desktop" as the default platform for ALL detected OS (macOS, Windows, Linux). Initial state is null until useEffect runs to avoid hydration mismatch.
...
- `useEffect` on mount: call `detectOS()`, set `detectedOS` state, set `mounted` to true, pre-select `"claude-desktop"` as default platform for ALL detected OS
