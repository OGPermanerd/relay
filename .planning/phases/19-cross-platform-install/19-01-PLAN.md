...
- Platform configs (all use `JSON.stringify(obj, null, 2)`):
  - **claude-desktop** (treated as Claude Desktop per INST-02): stdio config with `npx -y @relay/mcp` command. This is the Claude Desktop app that uses `claude_desktop_config.json`.
