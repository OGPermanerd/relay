...
1. Create `apps/mcp/src/tools/submit-for-review.ts`:
   ...
   - Handler (mirrors the web action logic):
     a. Check `getUserId()` -- return auth error if null.
     b. Check `!db` -- return DB error if null.
     c. Check `process.env.ANTHROPIC_API_KEY` -- return clear error if missing.
...
3. Ensure the duplicated `generateSkillReview` and `hashContent` are either:
   - Exported from `review-skill.ts` and imported here, OR
   - Extracted into a local `apps/mcp/src/tools/ai-review-helpers.ts` shared file if cleaner.
   The key constraint: MCP tools cannot import from `apps/web`. Keep it self-contained within `apps/mcp`.
