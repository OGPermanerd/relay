...
6. On success: call `upsertSkillReview()` with the result, then call `checkAutoApprove(categories)`.
   - If auto-approved: set status to `"published"` directly (the state machine allows `pending_review -> ai_reviewed -> approved -> published`, but we do one DB update with final status since the intermediate states are not observable to the user in this synchronous flow).
   - If not auto-approved: set status to `"ai_reviewed"`.
...
IMPORTANT: For auto-approve, transition directly to `"published"` in one update. The research confirms: `ai_reviewed -> approved -> published` intermediate states are not needed when it's programmatic auto-approve within a single action.
  </action>
  <verify>
