...
This follows the existing pattern of db:seed using tsx.
  ...
  <verify>
- `pnpm --filter @everyskill/db db:backfill-embeddings --help` doesn't error on tsx not found
...
Command:
```bash
pnpm --filter @everyskill/db db:backfill-embeddings
```
...
<verification>
1. Backfill script is executable: `pnpm --filter @everyskill/db db:backfill-embeddings`
...
3. TypeScript compilation passes: `pnpm --filter @everyskill/db typecheck`
</verification>
