...
Note: Run `pnpm --filter @everyskill/db db:push` after creating schema to apply changes
...
Commands:
1. `pnpm --filter @everyskill/db db:push` - Apply schema changes
...
- `pnpm --filter @everyskill/db db:studio` shows skill_embeddings table
...
<verification>
1. TypeScript compilation passes: `pnpm --filter @everyskill/db typecheck`
2. Schema is exported from @everyskill/db package
...
- Schema integrates with existing @everyskill/db package exports
</success_criteria>
