...
1. `getSkillsUsed(userId: string, limit = 20, offset = 0)`:
   ...
   SQL: SELECT from usage_events ue LEFT JOIN skills s ON s.id = ue.skill_id WHERE ue.user_id = userId, ORDER BY ue.created_at DESC, LIMIT/OFFSET. Use `COUNT(*) OVER()` window function for total. Return: skillId, skillName, category, action (toolName), timestamp (createdAt), hoursSaved (COALESCE(s.hours_saved, 1)).
...
2. `getSkillsUsedStats(userId: string)`:
   ...
   SQL: Single query with subquery for most-used skill. COUNT(DISTINCT skill_id), SUM(COALESCE(s.hours_saved, 1)), COUNT(*).
