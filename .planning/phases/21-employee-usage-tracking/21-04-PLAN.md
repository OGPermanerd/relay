...
<task type="auto">
  ...
  <action>
Create `apps/web/lib/my-leverage.ts` with these 4 functions. Follow the exact pattern from `leaderboard.ts`: import `db` from `@everyskill/db`, `sql` from `drizzle-orm`, use `db.execute(sql\`...\`)` for raw SQL, cast results as `unknown as Record<string, unknown>[]`.
