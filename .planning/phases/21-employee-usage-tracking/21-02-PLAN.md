...
1. Create `apps/web/app/api/install-callback/route.ts`:
   ...
   - Import `db` from `@everyskill/db`, `usageEvents` from `@everyskill/db/schema`
   ...
   - Also import and call `incrementSkillUses(skillId)` from `@everyskill/db/services/skill-metrics` when skillId is provided (same pattern as trackUsage in MCP)
