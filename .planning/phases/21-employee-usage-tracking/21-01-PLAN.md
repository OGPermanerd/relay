...
must_haves:
  truths:
    - "When EVERYSKILL_API_KEY is set, trackUsage() calls include userId"
    - "When no EVERYSKILL_API_KEY, tools work identically to before (anonymous)"
...
1. Create `apps/mcp/src/auth.ts`:
   ...
   - `resolveUserId()`: reads `process.env.EVERYSKILL_API_KEY`, calls `validateApiKey()`, caches result in module-scoped `cachedUserId`. Uses `resolved` flag so it runs at most once. Returns `string | null`. Use `console.error` for ALL logging (never console.log -- corrupts stdio).
...
3. Update each tool handler (search.ts, list.ts, deploy.ts):
   ...
   - Check `shouldNudge()` -- if true, push text content item: `"\n---\nTip: Set EVERYSKILL_API_KEY to track your personal skill leverage. Generate one at your Relay profile page."`
  </action>
  <verify>
