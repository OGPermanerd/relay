...
Purpose: While the SQL migrations (Plan 04) create the actual database policies, the Drizzle schema definitions must also declare them so Drizzle's introspection and future migration generation stay in sync. This ensures defense-in-depth: both database-level RLS and schema-level documentation of policies.
...
<task type="auto">
  ...
  <action>
Read each of the 9 schema files (they were modified by Plan 03 to include tenantId columns). Add pgPolicy definitions to each table.
