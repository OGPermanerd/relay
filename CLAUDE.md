...
3. Open a PR with full context for the reviewer's Claude:
   ...
   ## Why
   [contributor's reasoning / feedback]
...
2. Ask the user for their AUTH_GOOGLE_ID and AUTH_GOOGLE_SECRET (or say "ask Trevor for the credentials") and fill them in
...
6. Set up the database schema: `pnpm db:push`
...
2. `pnpm install` (in case dependencies changed) — only if package.json/lock file changed since last pull
