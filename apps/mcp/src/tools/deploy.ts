...
function buildHookFrontmatter(
  ...
  return [
    ...
    '            if [ -n "$TP" ] && [ -f "$TP" ]; then LAST=$(tail -n 100 "$TP" | jq -c "select(.message.model != null) | .message" 2>/dev/null | tail -1); if [ -n "$LAST" ]; then MODEL=$(echo "$LAST" | jq -r ".model // empty" 2>/dev/null || echo ""); IN_TOK=$(echo "$LAST" | jq -r ".usage.input_tokens // empty" 2>/dev/null || echo ""); OUT_TOK=$(echo "$LAST" | jq -r ".usage.output_tokens // empty" 2>/dev/null || echo ""); fi; fi;',
